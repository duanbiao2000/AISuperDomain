...
public partial class MainPage : ContentPage
{
    ...
    public MainPage()
    {
        ...
        dicConfiguration = new Dictionary<string, string[]>()
        {
            ...
            },
            {
                ...
                new string[]
                {
                    "https://neice.tiangong.cn/interlocutionPage",
                    js_rightClick,
                    "",
                    js_tiangong,
                    js_poe_getContent,
            ...
            {
                "xunfeixinghuo",
                new string[] { "https://xinghuo.xfyun.cn/desk", "", "", js_xinghuo, "" }
            },
            {
                "chatglm",
                new string[] { "https://chatglm.cn/detail", js_rightClick, "", js_chatglm, "" }
            },
            {
                "doubao",
                new string[] { "https://www.doubao.com/chat/", js_rightClick, "", js_doubao, "" }
            },
            {
                "baiduAI",
                new string[]
            ...
            {
                "agentGPT",
                new string[] { "https://agentgpt.reworkd.ai/zh", js_rightClick, "", "", "" }
            },
            {
                "autoGPT",
                new string[] { "https://autogpt.thesamur.ai/", js_rightClick, "", "", "" }
            },
            { "Skype", new string[] { "https://web.skype.com/", js_rightClick, "", "", "" } },
            {
                "Deepl",
                new string[] { "https://www.deepl.com/zh/translator", js_rightClick, "", "", "" }
            },
            { "bazi", new string[] { "https://ok.stoeng.site/hibazi", js_rightClick, "", "", "" } },
            {
                "ziwei",
                new string[] { "https://ok.stoeng.site/hiziwei", js_rightClick, "", "", "" }
            },
            {
                "stableaudio",
                new string[] { "https://stableaudio.com/generate", js_rightClick, "", "", "" }
            },
            {
                "stable-diffusion-XL",
                new string[] { "https://clipdrop.co/stable-diffusion", js_rightClick, "", "", "" }
            },
        };
        ...
        {
            ["多AI文章整合"] =
                "请基于下面这两篇文章的主题、信息和情节等，将两篇文章进行整合修改，重新生成一篇新的文章，要做到将两篇文章融合为一篇新的原创文章，但不要直接复制任何一篇文章中的句子，要做到新文章的句子和原文里的句子不重复，并改变句子顺序，在原文基础上增加一些细节，使得文章内容更加充实、全面，要做到100%原创。并且做到将原文中的词语替换为同义词或近义词，以进一步增强原创性，并且保证新文章保持语言流畅，内容合理：\r\n",
            ["机器学习"] =
            ...
            ["Linux 终端"] =
                "我希望你能模拟一个 Linux 终端。我会输入命令，你会回答终端应该显示什么。我要求你仅在一个唯一的代码块内回答终端输出，不要写解释，除非我指示你这样做。当我需要用英语告诉你一些内容时，我会用花括号 {像这样} 将文本括起来。我的第一个命令是：\r\n",
            ["英语翻译和修正"] =
                "我希望你能模拟英语翻译、拼写修正和改进。我会用任何语言和你交流，你会检测语言，翻译它并用更优美、更优雅、更高层次的英语单词和句子替换我的简单 A0 级别的单词和句子。保持意思相同，但使它们更具文学性。我要求你仅回答修正和改进，不要写解释。我的第一个句子是：\r\n",
            ["模拟面试官"] =
                "我希望你能扮演一名面试官。我将作为候选人，你将问我关于 position 职位的面试问题。我要求你仅回答作为面试官的问题。不要一次性写下所有的交流。像面试官一样，逐个提问并等待我的答案。不要写解释。一个一个地问我问题，像面试官一样，并等待我的答案。我的第一个句子是：\r\n",
            ["充当旅游指南"] =
                "我希望你能充当旅游指南。我会告诉你我的位置，你会建议我附近的游览景点。在某些情况下，我还会告诉你我想参观的类型。你还会向我建议附近类型相似的地方。我的第一个请求是：\r\n",
            ["充当抄袭检测器"] =
                "我希望你能充当抄袭检测器。我会写一些句子，你只需回复在给定句子的语言中未被检测出的抄袭，不能写解释。我的第一个句子是：\r\n",
        };
        ...
        if (_windows_count == "0")
        {
            webViewGroups = Enumerable
                .Range(0, 12)
                .Select(i =>
                    Enumerable.Range(1, 2).Select(j => webViewsDic[$"webView_{i * 2 + j}"]).ToList()
                )
                .ToList();
        }
        else if (_windows_count == "1")
        {
            webViewGroups = Enumerable
                .Range(0, 6)
                .Select(i =>
                    Enumerable.Range(1, 4).Select(j => webViewsDic[$"webView_{i * 4 + j}"]).ToList()
                )
                .ToList();
        }
        else
        {
            webViewGroups = Enumerable
                .Range(0, 4)
                .Select(i =>
                    Enumerable.Range(1, 6).Select(j => webViewsDic[$"webView_{i * 6 + j}"]).ToList()
                )
                .ToList();
        }
        ...
        editor1.TextChanged += async (sender, e) =>
        {
            ...
            if (
                ...
                && e.NewTextValue.StartsWith("@")
                && e.NewTextValue.Length >= 4
            )
            {
                ...
                if (action != null && action != "关闭")
                {
                    ...
                    //将要执行发送的代码加入list里
                    listCommonJs.Add(
                        dicConfiguration.TryGetValue(action, out var value) && value?.Length > 3
                            ? value[3]
                            : null
                    );
                    webView_common.Source = new Uri(
                        dicConfiguration.TryGetValue(action, out var value1) ? value1[0] : null
                    );

                    string _pre_js =
                        dicConfiguration.TryGetValue(action, out var uniqueValue)
                        && uniqueValue.Length > 2
                            ? uniqueValue[1] + uniqueValue[2]
                            : null;
    ...
    private void LoadAiUrl(List<string> listPlatform)
    {
        ...
        myPicker.ItemsSource = listPlatform
            ...
            .Select(g => string.Join(", ", g.Select(x => x.item.ToString())))
            .ToList();
        ...
        foreach (var aiName in listPlatform)
        {
            ...
            webViews[currentWebViewIndex].Navigated += async (s, e) =>
            {
                ...
                await webViews[currentWebViewIndex]
                    .EvaluateJavaScriptAsync(aiConfig[1] + aiConfig[2]);
            };
    ...
    private async void OnMyPopupPromptsSettingConfirmClicked(object sender, EventArgs e)
    {
        ...
        {
            if (
                rowLayout.Children[0] is Editor keyEditor
                && rowLayout.Children[1] is Editor valueEditor
            )
            {
    ...
    private async void OnWebViewCopyClicked(object sender, EventArgs e)
    {
        ...
        {
            string js =
                "function copySelectedText() { var selectedText = window.getSelection().toString(); if (selectedText !== '') { var el = document.createElement('textarea'); el.value = selectedText; document.body.appendChild(el); el.select(); document.execCommand('copy'); document.body.removeChild(el); return '已复制到剪贴板'; } else { return '未选定任何文本'; }}; copySelectedText();";
    ...
    private async void TranslateAndSendButton_Clicked(object sender, EventArgs e)
    {
        if (editor1.Text != null)
        {
            ...
            string url =
                $@"https://translate.googleapis.com/translate_a/single?dt=t&dt=bd&dt=qc&dt=rm&dt=ex&client=gtx&hl=en&sl=auto&tl=en&q={encode}&dj=1";
            ...
            catch (Exception ex)
            {
                await MainThread.InvokeOnMainThreadAsync(async () =>
                {
                    await Application.Current.MainPage.DisplayAlert(
                        "网络异常提示",
                        "无法访问谷歌翻译接口，请检查你的梯子魔法是否是全局模式。详情: "
                            + ex.Message,
                        "确定"
                    );
                });
    ...
    private async void GenerateImageStableDifussionButtonClick(object sender, EventArgs e)
    {
        ...
        webView_common.Source = new Uri(
            "https://runwayml-stable-diffusion-v1-5.hf.space/?__theme=light"
        );
    ...
    private async void OnStableDiffusionNavigated(object s, WebNavigatedEventArgs e)
    {
        ...
        string[] lines = imgPrompts.Split(new[] { "\r\n", "\r", "\n" }, StringSplitOptions.None);
        ...
        if (e.Result == WebNavigationResult.Success)
        {
            ...
            {
                var script =
                    "let e = document.querySelector('gradio-app').shadowRoot.querySelector('div').querySelector('input'); e.value = '[prompt]'; let t = new Event('input', { bubbles: true, cancelable: true }); e.dispatchEvent(t); let n = new KeyboardEvent('keydown', { bubbles: true, cancelable: true, key: 'Enter' }); e.dispatchEvent(n); document.querySelector('gradio-app').shadowRoot.querySelector('div').querySelector('button').click();".Replace(
                        "[prompt]",
                        line
                    );
                ...
                string jsCode =
                    "let elements = document.querySelector('gradio-app').shadowRoot.querySelector('div').querySelectorAll('button > img');"
                    + "let result = [];"
...
";
    ...
    {
        int claude2Index = File.ReadLines(filePath)
            .Any(line => line.Contains("\"OfficialClaude2\""))
            ? File.ReadLines(filePath)
                .TakeWhile(line => !line.Contains("\"OfficialClaude2\""))
                .Count() - 1
            : -1;
        ...
        {
            _getContentJS.Add(
                dicConfiguration.TryGetValue(_ai, out var uniqueValue) && uniqueValue.Length > 4
                    ? uniqueValue[4]
                    : null
            );
            _sendJs.Add(
                dicConfiguration.TryGetValue(_ai, out var value) && value?.Length > 3
                    ? value[3]
                    : null
            );
        }
        ...
        for (int i = 0; i < _content.Count; i += 2)
        {
            ...
            if (result1?.Equals("null") == true || result2?.Equals("null") == true)
            {
                await DisplayAlert(
                    "错误提示",
                    "未能获取到AI回答的内容，请确保AI已经回答完毕并且你能够看到AI给出的答案",
                    "ok"
                );
                editor1.Text = string.Empty;
                break;
